...
def file_in_use(filename='data.json', hash_val=0, des=False):
    old_data = {}
    ...
    if des:
        old_data['file_in_use'] = False
        old_data['file_in_use_hash'] = 0
    else:
        if old_data.get('file_in_use', False):
            if old_data.get('file_in_use_hash', 0) == hash_val:
                return True
            else:
                return False

        old_data['file_in_use'] = True
        old_data['file_in_use_hash'] = hash_val
